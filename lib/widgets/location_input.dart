...
class _LocationInputState extends State<LocationInput> {
  ...
  String get locationImage {
    ...
    // it generates a snapshot url that should be useable as a picture
  }

  Future<void> _savePlace(double latitude, double longitude) async {
    final url = Uri.parse(   // it converts url to uri
        'https://maps.googleapis.com/maps/api/geocode/json?latlng=$latitude,$longitude&key=AIzaSyDLcwxUggpPZo8lcbH0TB4Crq5SJjtj4ag');
    final response = await http.get(url);  // it is a jason file...so we have to convert into human readable formate
    final resData = json.decode(response.body);   // now it is in format of a map 
